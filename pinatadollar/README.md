...
```bash
pinatadollar$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pinatadollar$ sam local start-api
pinatadollar$ curl http://localhost:3000/
```
...
```bash
pinatadollar$ sam logs -n HelloWorldFunction --stack-name pinatadollar --tail
```
...
```bash
sam delete --stack-name pinatadollar
```
